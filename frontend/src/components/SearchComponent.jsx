...
export default function SearchComponent() {
  ...
  const performSearch = async (searchQuery) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/search/suggestions?q=${encodeURIComponent(searchQuery)}&limit=8`);
  ...
  const performFallbackSearch = async (searchQuery) => {
    try {
      const searchPromises = [
        fetch(`${API_BASE_URL}/product?search=${encodeURIComponent(searchQuery)}`).then(res => 
          res.ok ? res.json().then(data => ({ type: 'products', data: Array.isArray(data) ? data.slice(0, 3) : [] })) : { type: 'products', data: [] }
        ),
        fetch(`${API_BASE_URL}/brand`).then(res => 
  ...
  const handleSearchSelect = async (item) => {
    ...
    if (itemType === 'products' || itemType === 'product') {
      if (item.slug) {
        ...
        try {
          const response = await fetch(`${API_BASE_URL}/product?search=${encodeURIComponent(searchQuery)}`);
          if (response.ok) {
  ...
  const handleRecentSearchClick = async (recentItem) => {
    ...
    if (popularMatch) {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/search?q=${encodeURIComponent(searchQuery)}&limit=1`);
        if (response.ok) {
          const data = await response.json();
