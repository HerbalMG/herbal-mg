...
const statuses = ['All', 'Ordered', 'Shipped', 'Delivered', 'Replacement', 'Refunded', 'Cancelled'];
...
const OrderHistory = () => {
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      if (modalAction === 'replacement' && replacementImage) {
        toast.loading('Uploading replacement image...');
        ...
        try {
          ...
          replacementImageUrl = uploadData.imageUrl;
          toast.dismiss();
      ...
      // Update order status with replacement image
      await updateOrderStatus(modalOrder.id, newStatus, {
        replacement_image: replacementImageUrl,
        notes: modalAction === 'replacement' 
          ? `Replacement requested. Issues: ${replacementIssues.join(', ')}. Reason: ${replacementReason}`
          : undefined
      });
      ...
      toast.success(
        `${modalAction === 'cancel' ? 'Cancel' : 'Replacement'} request submitted successfully.`
      );
    } catch (error) {
  ...
  return (
    <div className="min-h-screen sm:p-6">
      <h1 className="text-2xl font-bold mb-6 text-center text-gray-800">Orders You’ve Placed</h1>
...
<div className="max-w-4xl mx-auto bg-white shadow-md rounded-md overflow-hidden">
                ...
                <div className="flex flex-col space-y-2">
                  {order.status === 'Ordered' && (
                    <button
                      ...
                      className="text-red-600 font-bold bg-amber-200 px-4 py-2 rounded hover:bg-amber-300 transition-colors whitespace-nowrap"
                    >
                      Cancel Order
                    </button>
                  )}
                  ...
                  {order.status === 'Delivered' && (
                    <button
                      ...
                      className={`px-4 py-2 rounded transition-colors whitespace-nowrap ${
                        returnEligible
                          ? 'text-purple-600 font-bold bg-gray-200 hover:bg-gray-300'
                          : 'text-gray-400 cursor-not-allowed bg-gray-100'
                      }`}
                    >
                      {returnEligible ? 'Replacement' : 'Expired'}
                    </button>
                  )}
                  
                  {!['Ordered', 'Delivered'].includes(order.status) && (
  ...
  <div className="sm:hidden">
          ...
          <div className="flex flex-col space-y-2 mt-3">
            {order.status === 'Ordered' && (
              <button
                ...
                className="w-full text-red-600 font-bold bg-amber-200 px-4 py-2 rounded hover:bg-amber-300 transition-colors"
              >
                Cancel Order
              </button>
            )}
            ...
            {order.status === 'Delivered' && (
              ...
              >
                {returnEligible ? 'Request Replacement' : 'Replacement Period Expired'}
              </button>
            )}
          </div>
    ...
    {filteredOrders.length === 0 && (
      <p className="text-center text-gray-500 py-6">You haven’t placed any order yet.</p>
    )}
  </div>
</div>
...
<div className="max-w-4xl mx-auto mt-6 flex justify-end">
  ...
  >
     You can reach out to:<span className='text-blue-600'>1234567898</span>
  </a>
</div>
