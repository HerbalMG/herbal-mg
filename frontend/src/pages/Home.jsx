...
const Home = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/product?seasonal_medicine=true`)
      .then((res) => res.json())
      .then((data) => {
    ...
    // Fetch top products
    fetch(`${API_BASE_URL}/product?top_products=true`)
      .then((res) => res.json())
      .then((data) => {
        if (Array.isArray(data)) {
          setTopProducts(data);
    ...
    fetch(`${API_BASE_URL}/product?people_preferred=true`)
      .then((res) => res.json())
      .then((data) => {
        if (Array.isArray(data)) {
    ...
    fetch(`${API_BASE_URL}/product?maximum_discount=true`)
      .then((res) => res.json())
      .then((data) => {
        if (Array.isArray(data)) {
  ...
  {error && (
    <div className="py-8 bg-yellow-50 rounded-lg border border-yellow-200">
      <div className="text-center px-4">
        ...
        <h3 className="text-lg font-semibold text-yellow-800 mb-2">Unable to Load Blogs</h3>
        <p className="text-sm text-yellow-600 mb-4">We're having trouble loading our latest articles. Please try again later.</p>
        <button
