...
export default function ProductDetails() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${API_BASE_URL}/product/${productIdentifier}`)
      .then((res) => {
...
fetch(`${API_BASE_URL}/product?category=${encodeURIComponent(data.category)}&exclude_id=${data.id}`)
  .then((res) => res.json())
  .then((similarData) => {
  ...
  .catch((err) => {
    ...
    setSimilarProducts([]);
        ...
        if (data.brand_id) {
          fetch(`${API_BASE_URL}/brand/${data.brand_id}`)
            .then((res) => res.json())
            .then((brandData) => {
        ...
        fetch(`${API_BASE_URL}/product_price/product/${data.id}`)
          .then((res) => {
            if (!res.ok) {
      ...
      <div className="text-red-600 text-sm mt-2">{prescriptionError}</div>
          ...
          <div className="space-y-1">
            ...
            onClick={async () => {
              ...
              if (prescriptionFile) {
                try {
                  toast.loading("Uploading prescription...");
