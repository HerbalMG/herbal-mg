...
function Ayurvedic() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/product?category=Ayurvedic`);
  ...
  const fetchFilterOptions = async () => {
    try {
      // Fetch brands
      const brandsResponse = await fetch(`${API_BASE_URL}/brand`)
      ...
      try {
        const diseasesResponse = await fetch(`${API_BASE_URL}/disease`)
  ...
  const applyFilters = () => {
    ...
    if (selectedBrand) {
      ...
      filtered = filtered.filter(product => {
        ...
        if (brandMatch) {
          console.log(`  ✅ Match: ${product.name}, brand_id: ${product.brand_id}`);
        }
        return brandMatch;
      })
    ...
    // Disease filter
    if (selectedDisease) {
      console.log('🔍 Filtering by disease:', selectedDisease, typeof selectedDisease);
      ...
      filtered = filtered.filter(product => {
        ...
        const match = nameMatch || descMatch || keyMatch;
        ...
        >
          <BiAbacus className='text-black w-7 h-7'/>
            ...
            <div className="flex items-center gap-2">
              {/* <label className="text-md font-medium text-gray-700">Sort by:</label> */}
              <select
                value={sortBy}
